...
export default function Dashboard() {
  ...
  const { data: latestConversationsData, isLoading: conversationsLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await apiRequest("GET", `/api/conversations?brandId=${activeBrandId}&limit=5`);
      return res.json();
    },
  ...
  const boostySuggestions = [
    { icon: Wand2, text: isSpanish ? "Crear post viral" : "Create viral post", action: "/content-planner" },
    { icon: Target, text: isSpanish ? "Optimizar campaña" : "Optimize campaign", action: "/campaigns" },
    { icon: MessageCircle, text: isSpanish ? "Responder mensajes" : "Reply to messages", action: "/inbox" },
  ];

  const platformStyles: Record<string, { icon: any; color: string; bgGradient: string; name: string }> = {
    facebook: { icon: SiFacebook, color: "#1877F2", bgGradient: "from-blue-500/10 to-blue-600/5", name: "Facebook" },
    instagram: { icon: SiInstagram, color: "#E4405F", bgGradient: "from-pink-500/10 to-purple-600/5", name: "Instagram" },
    whatsapp: { icon: SiWhatsapp, color: "#25D366", bgGradient: "from-green-500/10 to-green-600/5", name: "WhatsApp" },
    linkedin: { icon: SiLinkedin, color: "#0A66C2", bgGradient: "from-blue-600/10 to-blue-700/5", name: "LinkedIn" },
    tiktok: { icon: SiTiktok, color: "#000000", bgGradient: "from-gray-800/10 to-gray-900/5", name: "TikTok" },
    youtube: { icon: SiYoutube, color: "#FF0000", bgGradient: "from-red-500/10 to-red-600/5", name: "YouTube" },
  };
          ...
          <main className="flex-1 relative overflow-y-auto focus:outline-none">
            <div className="py-6 px-4 sm:px-6 lg:px-8">
              ...
              >
                ...
                <motion.div variants={fadeInUp} className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-brand-600 via-brand-500 to-purple-600 p-8 shadow-2xl">
                    ...
                    {hasPOS ? (
                        ...
                        <CardContent className="p-6">
                          <div className="flex items-start justify-between">
                            <div>
                              <p className="text-emerald-100 text-sm font-medium mb-1">
                                {isSpanish ? "Ingresos Totales" : "Total Revenue"}
                              </p>
                              <div className="flex items-baseline gap-2">
                                <span className="text-4xl font-bold">
                                  <AnimatedCounter 
                                    value={selectedPeriod === "daily" ? "1,890" : selectedPeriod === "weekly" ? "12,450" : "52,800"} 
                                    prefix="$" 
                                  />
                                </span>
                                <Badge className="bg-white/20 text-white border-0">
                                  <TrendingUp className="w-3 h-3 mr-1" />
                                  +{selectedPeriod === "daily" ? "12" : selectedPeriod === "weekly" ? "47" : "63"}%
                                </Badge>
                              </div>
                              <p className="text-emerald-100 text-sm mt-2">
                                {isSpanish ? "vs antes de Lead Boost" : "vs before Lead Boost"}
                              </p>
                            </div>
                          ...
                          <div className="mt-4 pt-4 border-t border-white/20">
                            <div className="flex items-center justify-between text-sm">
                              <span className="text-emerald-100">{isSpanish ? "Meta mensual" : "Monthly goal"}</span>
                      ...
                      <Card className="relative overflow-hidden border-2 border-dashed border-gray-200 bg-gradient-to-br from-gray-50 to-white h-full group hover:border-brand-300 transition-all">
                        <CardContent className="p-8 flex flex-col items-center justify-center text-center h-full">
                          <motion.div
                            animate={hoveredCard === 'revenue' ? { scale: 1.1, rotate: 5 } : { scale: 1, rotate: 0 }}
                    ...
                    {hasSocial ? (
                      <Card className="relative overflow-hidden border-0 shadow-lg bg-gradient-to-br from-violet-500 to-purple-600 text-white h-full">
                        ...
                        <CardContent className="p-6">
                          ...
                          <div className="flex items-baseline gap-2 mb-2">
                            <span className="text-4xl font-bold">
                              <AnimatedCounter value={selectedPeriod === "daily" ? 1 : selectedPeriod === "weekly" ? 7 : 28} />
                            </span>
                          </div>
                          <div className="flex items-center gap-1 text-violet-100 text-sm">
                            <Target className="w-4 h-4" />
                            <span>{selectedPeriod === "weekly" ? "147" : selectedPeriod === "monthly" ? "588" : "21"} posts</span>
                          </div>
                          <div className="mt-4">
                            <MiniSparkline data={campaignData} color="#ffffff" />
                          </div>
                        </CardContent>
                      </Card>
                    ) : (
                      <Card className="relative overflow-hidden border-2 border-dashed border-gray-200 bg-gradient-to-br from-gray-50 to-white h-full group hover:border-brand-300 transition-all">
                        <CardContent className="p-6 flex flex-col items-center justify-center text-center h-full">
                          <motion.div
                            animate={hoveredCard === 'campaigns' ? { scale: 1.1, rotate: -5 } : { scale: 1, rotate: 0 }}
                          ...
                          <p className="text-xs text-gray-500 mb-4">
                            {isSpanish ? "Conecta redes sociales" : "Connect social media"}
                          </p>
                          <Link href="/integrations">
                            <Button size="sm" variant="outline" className="text-xs border-violet-200 hover:bg-violet-50">
                    ...
                    {hasMessaging ? (
                      <Card className="relative overflow-hidden border-0 shadow-lg bg-gradient-to-br from-blue-500 to-indigo-600 text-white h-full">
                        ...
                        <CardContent className="p-6">
                          ...
                          <div className="flex items-baseline gap-2 mb-2">
                            <span className="text-4xl font-bold">
                              <AnimatedCounter value={latestConversations.reduce((acc: number, c: any) => acc + (c.unreadCount || 0), 0)} />
                            </span>
                          </div>
                          <div className="flex items-center gap-1 text-blue-100 text-sm">
                            <Users className="w-4 h-4" />
                            <span>{latestConversations.length} {isSpanish ? "conversaciones" : "conversations"}</span>
                          </div>
                          <div className="mt-4">
                      ...
                      <Card className="relative overflow-hidden border-2 border-dashed border-gray-200 bg-gradient-to-br from-gray-50 to-white h-full group hover:border-brand-300 transition-all">
                        <CardContent className="p-6 flex flex-col items-center justify-center text-center h-full">
                          <motion.div
                            animate={hoveredCard === 'messages' ? { scale: 1.1, rotate: 5 } : { scale: 1, rotate: 0 }}
                          ...
                          <p className="text-xs text-gray-500 mb-4">
                            {isSpanish ? "Conecta WhatsApp o IG" : "Connect WhatsApp or IG"}
                          </p>
                          <Link href="/integrations">
                            <Button size="sm" variant="outline" className="text-xs border-blue-200 hover:bg-blue-50">
                              <Plug className="w-3 h-3 mr-1" />
                              {isSpanish ? "Conectar" : "Connect"}
                            </Button>
                          </Link>
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <CardTitle className="text-lg font-semibold text-gray-900">
                                {isSpanish ? "Conversaciones Recientes" : "Recent Conversations"}
                              </CardTitle>
                              <p className="text-sm text-gray-500">
                                {isSpanish ? "Responde a tus clientes" : "Reply to your customers"}
                              </p>
                            </div>
                          </div>
                          <Link href="/inbox">
                        ...
                        {conversationsLoading ? (
                          ...
                          <div className="text-center py-16 px-6">
                            ...
                            <h3 className="text-xl font-semibold text-gray-900 mb-2">
                              {isSpanish ? "No tienes conversaciones aún" : "You don't have conversations yet"}
                            </h3>
                            ...
                            <Link href="/integrations">
                              <Button className="bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700 text-white shadow-lg">
                                <Plug className="w-4 h-4 mr-2" />
                                {isSpanish ? "Conectar plataformas" : "Connect platforms"}
                              </Button>
                            </Link>
                          </div>
                          ...
                          <div className="divide-y divide-gray-100">
                            {latestConversations.map((conversation: any, index: number) => {
                              const platform = platformStyles[conversation.platform] || {
                                icon: HelpCircle, color: "#9CA3AF", bgGradient: "from-gray-400/10 to-gray-500/5", name: "Unknown"
                              };
                              ...
                              return (
                                <motion.div
                                  ...
                                  role="button"
                                  tabIndex={0}
                                  onClick={() => (window.location.href = `/inbox?conversation=${conversation.id}`)}
                                  className="flex items-center gap-4 p-4 cursor-pointer transition-all"
                                >
                                  <div className={`flex-shrink-0 w-12 h-12 rounded-xl flex items-center justify-center bg-gradient-to-br ${platform.bgGradient} border border-gray-100`}>
                          ...
                          <div className="text-center py-12 px-6">
                            ...
                            <p className="text-gray-500 font-medium">
                              {isSpanish ? "No hay conversaciones recientes" : "No recent conversations"}
                            </p>
                            <p className="text-sm text-gray-400 mt-1">
                              {isSpanish ? "Las nuevas conversaciones aparecerán aquí" : "New conversations will appear here"}
                            </p>
                          </div>
                        )}
                      </CardContent>
                    ...
                    <Card className="overflow-hidden border-0 shadow-lg bg-gradient-to-br from-brand-50 via-white to-purple-50 h-full">
                      <CardHeader className="border-b border-brand-100/50 px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          >
                            <img src={boosty} alt="Boosty" className="w-12 h-12 rounded-full border-2 border-brand-200" />
                          </motion.div>
                          <div>
                            ...
                            <p className="text-sm text-gray-500">
                              {isSpanish ? "Tu asistente de marketing" : "Your marketing assistant"}
                            </p>
                          </div>
                        </div>
                      </CardHeader>
                      ...
                      <CardContent className="p-4 space-y-4">
                        ...
                        <div className="space-y-3">
                          <p className="text-xs font-medium text-gray-500 uppercase tracking-wide">
                            {isSpanish ? "Sugerencias para ti" : "Suggestions for you"}
                          </p>
                          
                          {[
                            {
                              icon: Rocket,
                              title: isSpanish ? "Crear post viral" : "Create viral post",
                              desc: isSpanish ? "Basado en tendencias actuales" : "Based on current trends",
                              action: "/content-planner",
                              color: "from-orange-500 to-red-500"
                            },
                            {
                              icon: Calendar,
                              title: isSpanish ? "Programar contenido" : "Schedule content",
                              desc: isSpanish ? "Optimiza tu calendario" : "Optimize your calendar",
                              action: "/content-planner",
                              color: "from-blue-500 to-indigo-500"
                            },
                            {
                              icon: BarChart3,
                              title: isSpanish ? "Ver analytics" : "View analytics",
                              desc: isSpanish ? "Métricas de engagement" : "Engagement metrics",
                              action: "/analytics",
                        ...
                        <Link href="/home">
                          <motion.button
                            ...
                            whileTap={{ scale: 0.98 }}
                            className="w-full py-3 px-4 bg-gradient-to-r from-brand-500 to-purple-600 hover:from-brand-600 hover:to-purple-700 text-white rounded-xl font-medium shadow-lg shadow-brand-500/25 transition-all flex items-center justify-center gap-2"
                          >
                      ...
                      <div className="flex items-center justify-between mb-6">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-gray-500">
                              {isSpanish ? "Completa estos pasos para maximizar tu éxito" : "Complete these steps to maximize your success"}
                            </p>
                          </div>
                        </div>
                        <Badge className="bg-amber-100 text-amber-700 border-0">
                          {hasPOS && hasSocial && hasMessaging ? "3/3" : hasPOS || hasSocial || hasMessaging ? `${[hasPOS, hasSocial, hasMessaging].filter(Boolean).length}/3` : "0/3"} {isSpanish ? "completado" : "complete"}
                        </Badge>
                      </div>
                        ...
                        {[
                          {
                            ...
                            desc: isSpanish ? "Ve tus ventas en tiempo real" : "See your sales in real-time",
                            done: hasPOS,
                            icon: ShoppingBag,
                            link: "/integrations"
                          },
                          {
                            title: isSpanish ? "Conectar redes sociales" : "Connect social media",
                            desc: isSpanish ? "Crea y programa contenido" : "Create and schedule content",
                            done: hasSocial,
                            icon: SiInstagram,
                            link: "/integrations"
                          },
                          {
                            title: isSpanish ? "Activar mensajería" : "Activate messaging",
                            desc: isSpanish ? "Responde a tus clientes" : "Reply to your customers",
                            done: hasMessaging,
                            icon: MessageSquare,
                            link: "/integrations"
                          }
                          ...
                          >
                            <div className="flex items-start gap-3">
                              ...
                              <div className="flex-1">
                                ...
                                {!step.done && (
                                  <Link href={step.link}>
                                    <Button size="sm" variant="ghost" className="text-brand-600 hover:text-brand-700 p-0 h-auto mt-2 text-xs">
                                      {isSpanish ? "Conectar" : "Connect"} <ArrowRight className="w-3 h-3 ml-1" />
                                    </Button>
                                  </Link>
                                )}
                              </div>
