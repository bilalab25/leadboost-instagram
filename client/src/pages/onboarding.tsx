...
const INTEGRATION_PROVIDERS = {
  square: { name: "Square", icon: CreditCard, description: "Point of sale and payment processing", category: "pos" },
  stripe: { name: "Stripe", icon: CreditCard, description: "Online payment processing", category: "pos" },
  shopify: { name: "Shopify", icon: ShoppingBag, description: "E-commerce platform", category: "ecommerce" },
  woocommerce: { name: "WooCommerce", icon: Globe, description: "WordPress e-commerce plugin", category: "ecommerce" },
  wix: { name: "Wix", icon: LayoutGrid, description: "Website builder and e-commerce platform", category: "ecommerce" },
  instagram: { name: "Instagram", icon: Instagram, description: "Connect your Instagram account for posts and analytics", category: "social_media" },
  facebook: { name: "Facebook", icon: Facebook, description: "Connect your Facebook Page for posts and insights", category: "social_media" },
  tiktok: { name: "TikTok", icon: Share2, description: "Connect your TikTok account for content scheduling", category: "social_media" },
  youtube: { name: "YouTube", icon: Youtube, description: "Connect your YouTube channel for video management", category: "social_media" },
  hubspot: { name: "HubSpot", icon: BriefcaseBusiness, description: "Connect your HubSpot CRM to manage leads and customers", category: "crm" },
};
...
const INTEGRATION_CATEGORIES = {
  ...
  ecommerce: { name: "Website & E-commerce", icon: ShoppingBag, description: "Integrate your website or online store." },
  crm: { name: "CRM Systems", icon: BriefcaseBusiness, description: "Link your CRM to centralize customer data." },
};
...
const getAssetType = (fileName: string): BrandAsset["assetType"] => {
  ...
  if (["jpeg", "jpg", "png", "gif", "svg", "webp"].includes(ext)) return "image";
      ...
      {steps.slice(0, totalSteps).map((step, index) => (
        <div key={step.number} className="flex items-center">
          <div className="flex flex-col items-center">
            ...
            <span className={`text-xs mt-1 hidden sm:block ${currentStep === step.number ? "text-indigo-600 font-medium" : "text-gray-500"}`}>
              {step.label}
            </span>
          </div>
          {index < steps.slice(0, totalSteps).length - 1 && (
            <div className={`w-12 sm:w-20 h-1 mx-2 ${currentStep > step.number ? "bg-green-500" : "bg-gray-200"}`} />
          )}
        </div>
      ))}
    </div>
...
export default function Onboarding() {
  ...
  const { refreshBrands, brands, setActiveBrandId } = useBrand();
  ...
  const [customFontFiles, setCustomFontFiles] = useState<{ name: string; url: string; family: string }[]>([]);
  const [customFontOptions, setCustomFontOptions] = useState<string[]>([]);
  
  // Logo states
  const [whiteLogoFile, setWhiteLogoFile] = useState<File | null>(null);
  const [whiteLogoPreviewUrl, setWhiteLogoPreviewUrl] = useState<string | null>(null);
  const [blackLogoFile, setBlackLogoFile] = useState<File | null>(null);
  const [blackLogoPreviewUrl, setBlackLogoPreviewUrl] = useState<string | null>(null);
  const [whiteFaviconFile, setWhiteFaviconFile] = useState<File | null>(null);
  const [whiteFaviconPreviewUrl, setWhiteFaviconPreviewUrl] = useState<string | null>(null);
  const [blackFaviconFile, setBlackFaviconFile] = useState<File | null>(null);
  const [blackFaviconPreviewUrl, setBlackFaviconPreviewUrl] = useState<string | null>(null);

  // Asset states
  type UploadItem = { id: string; name: string; percent: number };
  const [uploads, setUploads] = useState<UploadItem[]>([]);
  const [currentAssetUploadCategory, setCurrentAssetUploadCategory] = useState<string>(assetCategories[0].value);
  ...
  const { data: brandDesign, isLoading: isDesignLoading } = useQuery<BrandDesign>({
  ...
  // Query assets
  const { data: assets = [], isLoading: isAssetsLoading } = useQuery<BrandAsset[]>({
  ...
  useEffect(() => {
    if (brandDesign) {
      if (brandDesign.colorPalette || brandDesign.colorPrimary) {
        setMainColor(brandDesign.colorPalette?.primary || brandDesign.colorPrimary || "#2563eb");
        setAccentColor1(brandDesign.colorPalette?.accent1 || brandDesign.colorAccent1 || "#60a5fa");
        setAccentColor2(brandDesign.colorPalette?.accent2 || brandDesign.colorAccent2 || "#1e40af");
        setText1Color(brandDesign.colorPalette?.text1 || brandDesign.colorText1 || "#333333");
        setText2Color(brandDesign.colorPalette?.text2 || brandDesign.colorText2 || "#666666");
      }
      if (brandDesign.typography || brandDesign.fontPrimary) {
        setPrimaryFont(brandDesign.typography?.primary || brandDesign.fontPrimary || "Roboto");
        setSecondaryFont(brandDesign.typography?.secondary || brandDesign.fontSecondary || "Open Sans");
      }
  ...
  const createBrandMutation = useMutation({
    ...
    onSuccess: (data) => {
      toast({
        ...
        description: isSpanish ? "Tu marca ha sido creada exitosamente." : "Your brand has been created successfully.",
      });
    ...
    onError: (error: any) => {
      toast({
        ...
        description: error.message || (isSpanish ? "No se pudo crear la marca" : "Failed to create brand"),
        variant: "destructive",
      });
    },
  });
  ...
  const joinBrandMutation = useMutation({
    ...
    onSuccess: () => {
      toast({
        ...
        description: isSpanish ? "Te has unido exitosamente a la marca." : "You've successfully joined the brand.",
      });
      queryClient.invalidateQueries({ queryKey: ["/api/brand-memberships"] });
      refreshBrands();
    ...
    onError: (error: any) => {
      toast({
        title: isSpanish ? "Error" : "Error",
        description: error.message || (isSpanish ? "No se pudo unir a la marca" : "Failed to join brand"),
        variant: "destructive",
      });
    },
  });
  ...
  const saveBrandDesignMutation = useMutation({
    mutationFn: async (designData: any) => {
      ...
      const response = await apiRequest("POST", `/api/brand-design?brandId=${createdBrandId}`, designData);
      if (!response.ok) {
    ...
    onSuccess: () => {
      queryClientInstance.invalidateQueries({ queryKey: ["/api/brand-design", createdBrandId] });
      toast({
  ...
  function uploadFileWithProgress(file: File, onProgress: (pct: number) => void): Promise<any> {
  ...
  const saveAssetToDB = async (asset: BrandAsset) => {
    ...
    await apiRequest("POST", `/api/brand-assets?brandId=${createdBrandId}`, payload);
  };

  const handleRemoveAsset = async (assetId: string) => {
    try {
      await apiRequest("DELETE", `/api/brand-assets/${assetId}?brandId=${createdBrandId}`);
      await queryClientInstance.invalidateQueries({
        queryKey: ["/api/brand-assets", createdBrandId, brandDesign?.id],
      });
      toast({
        title: isSpanish ? "Eliminado" : "Deleted",
        description: isSpanish ? "Recurso eliminado correctamente" : "Asset deleted successfully",
      });
    } catch (err) {
      toast({
        title: isSpanish ? "Error" : "Error",
        description: isSpanish ? "No se pudo eliminar el recurso" : "Failed to delete asset",
        variant: "destructive",
      });
    }
  };
  ...
  const handleSaveBrandDesign = async (): Promise<boolean> => {
    try {
      ...
      const whiteLogoUrl = await uploadIfNeeded(whiteLogoFile, whiteLogoPreviewUrl, setWhiteLogoPreviewUrl, "white logo");
      const blackLogoUrl = await uploadIfNeeded(blackLogoFile, blackLogoPreviewUrl, setBlackLogoPreviewUrl, "black logo");
      const whiteFaviconUrl = await uploadIfNeeded(whiteFaviconFile, whiteFaviconPreviewUrl, setWhiteFaviconPreviewUrl, "white favicon");
      const blackFaviconUrl = await uploadIfNeeded(blackFaviconFile, blackFaviconPreviewUrl, setBlackFaviconPreviewUrl, "black favicon");
      ...
      const designData: Record<string, any> = {
        ...
        typography: {
          ...
          customFonts: customFontFiles.map((f) => ({ name: f.family, url: f.url })),
        },
      ...
      const errorMessage = err?.message || (isSpanish ? "No se pudo guardar el diseño" : "Failed to save brand design");
      toast({
        title: isSpanish ? "Error" : "Error",
  ...
  const handleFinishOnboarding = async () => {
    toast({
      ...
      description: isSpanish ? "Tu marca está lista para usar." : "Your brand is ready to use.",
    });
    setLocation("/dashboard");
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 dark:text-gray-300">{isSpanish ? "Cargando..." : "Loading..."}</p>
        </div>
      </div>
    );
  }
        ...
        <div className="w-full max-w-4xl">
          <div className="text-center mb-8">
            ...
            <p className="text-lg text-gray-600 dark:text-gray-300">
              {isSpanish ? "Empecemos con tu marca" : "Let's get you started with your brand"}
            </p>
          </div>
            ...
            >
              <CardHeader>
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <CardTitle className="text-2xl">{isSpanish ? "Crear una Marca" : "Create a Brand"}</CardTitle>
                </div>
              ...
              <CardContent>
                <ul className="space-y-2 text-sm text-gray-600 dark:text-gray-300">
                  <li className="flex items-center gap-2">
                    ...
                    {isSpanish ? "Posee y administra tu marca" : "Own and manage your brand"}
                  </li>
                  <li className="flex items-center gap-2">
                    <div className="w-1.5 h-1.5 bg-indigo-500 rounded-full" />
                    {isSpanish ? "Invita miembros del equipo" : "Invite team members"}
                  </li>
                  <li className="flex items-center gap-2">
                    <div className="w-1.5 h-1.5 bg-indigo-500 rounded-full" />
                    {isSpanish ? "Control total sobre configuraciones" : "Full control over settings"}
                  </li>
                </ul>
              </CardContent>
            </Card>
            ...
            >
              <CardHeader>
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <CardTitle className="text-2xl">{isSpanish ? "Unirse a una Marca" : "Join a Brand"}</CardTitle>
                </div>
                <CardDescription className="text-base">
                  {isSpanish
              ...
              <CardContent>
                <ul className="space-y-2 text-sm text-gray-600 dark:text-gray-300">
                  <li className="flex items-center gap-2">
                    ...
                    {isSpanish ? "Colabora con tu equipo" : "Collaborate with your team"}
                  </li>
                  <li className="flex items-center gap-2">
                    <div className="w-1.5 h-1.5 bg-purple-500 rounded-full" />
                    {isSpanish ? "Accede a recursos compartidos" : "Access shared resources"}
                  </li>
                  <li className="flex items-center gap-2">
                    <div className="w-1.5 h-1.5 bg-purple-500 rounded-full" />
                    {isSpanish ? "Comienza a trabajar inmediatamente" : "Start working immediately"}
                  </li>
                </ul>
              </CardContent>
            </Card>
            ...
            <Form {...joinForm}>
              <form onSubmit={joinForm.handleSubmit(onJoinSubmit)} className="space-y-4">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>{isSpanish ? "Código de Invitación *" : "Invitation Code *"}</FormLabel>
                      <FormControl>
                        <Input
                          {...field}
                          placeholder={isSpanish ? "Ingresa el código de invitación" : "Enter invitation code"}
                ...
                <div className="flex gap-3 pt-4">
                  ...
                  >
                    {joinBrandMutation.isPending
                      ? isSpanish ? "Uniéndose..." : "Joining..."
                      : isSpanish ? "Unirse" : "Join Brand"}
                  </Button>
                </div>
              </form>
            </Form>
      ...
      <div className="max-w-4xl mx-auto">
        ...
        {currentStep === 1 && (
          <Card className="w-full max-w-lg mx-auto">
            <CardHeader>
              <CardTitle className="text-2xl">{isSpanish ? "Crear Tu Marca" : "Create Your Brand"}</CardTitle>
              <CardDescription>
                {isSpanish ? "Configura el perfil de tu marca para comenzar" : "Set up your brand profile to get started"}
              </CardDescription>
            </CardHeader>
            <CardContent>
              <Form {...createForm}>
                <form onSubmit={createForm.handleSubmit(onCreateSubmit)} className="space-y-4">
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>{isSpanish ? "Nombre de la Marca *" : "Brand Name *"}</FormLabel>
                        <FormControl>
                          <Input {...field} placeholder="e.g. Acme Corp" data-testid="input-brand-name" />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>{isSpanish ? "Industria" : "Industry"}</FormLabel>
                        <FormControl>
                          <Input
                            {...field}
                            placeholder={isSpanish ? "ej. Tecnología, Retail, Salud" : "e.g. Technology, Retail, Healthcare"}
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Textarea
                            {...field}
                            placeholder={isSpanish ? "Cuéntanos sobre tu marca..." : "Tell us about your brand..."}
                            rows={3}
          ...
          <Card>
            ...
            <CardContent className="space-y-6">
              {isDesignLoading ? (
                ...
                <Accordion type="multiple" defaultValue={["style", "colors", "logos", "typography"]} className="space-y-4">
                    ...
                    <AccordionContent>
                      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 pt-4">
                        ...
                        <div className="space-y-2">
                          <Label>{isSpanish ? "Logo Claro" : "White Logo"}</Label>
                          <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center">
                            {whiteLogoPreviewUrl ? (
                              <div className="flex flex-col items-center">
                                <img src={whiteLogoPreviewUrl} alt="White Logo" className="max-h-20 object-contain mb-2" />
                                <Button
                                  variant="ghost"
                                  size="sm"
                                  onClick={() => {
                                ...
                                >
                                  <Trash2 className="h-4 w-4 mr-1" /> {isSpanish ? "Eliminar" : "Remove"}
                                </Button>
                              </div>
                            ) : (
                              <>
                                <Upload className="mx-auto h-8 w-8 text-gray-400" />
                                <Label htmlFor="white-logo" className="cursor-pointer">
                                  ...
                                  <input
                                    ...
                                    accept="image/*"
                                    className="sr-only"
                                    onChange={(e) => handleFileUpload(e, setWhiteLogoFile, setWhiteLogoPreviewUrl)}
                                  />
                                </Label>
                              </>
                            )}
                        ...
                        {/* Black Logo */}
                        <div className="space-y-2">
                          <Label>{isSpanish ? "Logo Oscuro" : "Black Logo"}</Label>
                          <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center">
                            {blackLogoPreviewUrl ? (
                              <div className="flex flex-col items-center">
                                <img src={blackLogoPreviewUrl} alt="Black Logo" className="max-h-20 object-contain mb-2" />
                                <Button
                                  variant="ghost"
                                  size="sm"
                                  onClick={() => {
                              ...
                              <>
                                <Upload className="mx-auto h-8 w-8 text-gray-400" />
                                <Label htmlFor="black-logo" className="cursor-pointer">
                                  <span className="font-medium text-indigo-600 hover:text-indigo-500">
                                    {isSpanish ? "Subir logo" : "Upload logo"}
                                  </span>
                                  <input
                                    ...
                                    accept="image/*"
                                    className="sr-only"
                                    onChange={(e) => handleFileUpload(e, setBlackLogoFile, setBlackLogoPreviewUrl)}
                                  />
                                </Label>
                              </>
                            )}
                        ...
                        {/* White Favicon */}
                        <div className="space-y-2">
                          <Label>{isSpanish ? "Favicon Claro" : "White Favicon"}</Label>
                          <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center">
                            {whiteFaviconPreviewUrl ? (
                              <div className="flex flex-col items-center">
                                <img src={whiteFaviconPreviewUrl} alt="White Favicon" className="max-h-16 object-contain mb-2" />
                                <Button
                                  variant="ghost"
                                  size="sm"
                                  onClick={() => {
                              ...
                              <>
                                <Upload className="mx-auto h-8 w-8 text-gray-400" />
                                <Label htmlFor="white-favicon" className="cursor-pointer">
                                  <span className="font-medium text-indigo-600 hover:text-indigo-500">
                                    {isSpanish ? "Subir favicon" : "Upload favicon"}
                                  </span>
                                  <input
                                    id="white-favicon"
                                    type="file"
                                    accept="image/*"
                                    className="sr-only"
                                    onChange={(e) => handleFileUpload(e, setWhiteFaviconFile, setWhiteFaviconPreviewUrl)}
                                  />
                                </Label>
                              </>
                            )}
                        ...
                        {/* Black Favicon */}
                        <div className="space-y-2">
                          <Label>{isSpanish ? "Favicon Oscuro" : "Black Favicon"}</Label>
                          <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center">
                            {blackFaviconPreviewUrl ? (
                              <div className="flex flex-col items-center">
                                <img src={blackFaviconPreviewUrl} alt="Black Favicon" className="max-h-16 object-contain mb-2" />
                                <Button
                                  variant="ghost"
                                  size="sm"
                                  onClick={() => {
                              ...
                              <>
                                <Upload className="mx-auto h-8 w-8 text-gray-400" />
                                <Label htmlFor="black-favicon" className="cursor-pointer">
                                  <span className="font-medium text-indigo-600 hover:text-indigo-500">
                                    {isSpanish ? "Subir favicon" : "Upload favicon"}
                                  </span>
                                  <input
                                    ...
                                    accept="image/*"
                                    className="sr-only"
                                    onChange={(e) => handleFileUpload(e, setBlackFaviconFile, setBlackFaviconPreviewUrl)}
                                  />
                                </Label>
                              </>
                            )}
                    ...
                    <AccordionContent>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4">
                        <div className="space-y-2">
                          ...
                          />
                          <p className="text-sm text-gray-500" style={{ fontFamily: primaryFont }}>
                            {isSpanish ? "Vista previa de la fuente principal" : "Preview of primary font"}
                          </p>
                        </div>
                        <div className="space-y-2">
                          ...
                          />
                          <p className="text-sm text-gray-500" style={{ fontFamily: secondaryFont }}>
                            {isSpanish ? "Vista previa de la fuente secundaria" : "Preview of secondary font"}
                          </p>
                        </div>
                      </div>
                    </AccordionContent>
              ...
              <div className="flex gap-3 pt-6">
                <Button type="button" variant="outline" onClick={handlePrevStep} className="flex-1" data-testid="button-prev-step-2">
                  <ArrowLeft className="w-4 h-4 mr-2" />
                ...
                >
                  {saveBrandDesignMutation.isPending ? (isSpanish ? "Guardando..." : "Saving...") : isSpanish ? "Siguiente" : "Next"}
                  <ArrowRight className="w-4 h-4 ml-2" />
                </Button>
              </div>
            </CardContent>
        ...
        {currentStep === 3 && (
          <Card>
            ...
            <CardContent className="space-y-6">
                ...
                <>
                  ...
                  <Accordion type="multiple" className="w-full">
                    {assetCategories.map((category) => {
                      const assetsInCategory = assets.filter((asset) => asset.category === category.value);
                          ...
                          <AccordionContent className="space-y-4">
                            ...
                            <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center">
                              ...
                              <div className="mt-3">
                                ...
                                <p className="text-sm text-gray-500 mt-1">
                                  {isSpanish ? "Imágenes, videos o PDFs." : "Images, videos or PDFs."}
                                </p>
                              </div>
                            </div>
                            ...
                            {assetsInCategory.length > 0 ? (
                                ...
                                {assetsInCategory.map((asset) => (
                                  ...
                                  >
                                    ...
                                    <div className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                                      ...
                                      <a href={asset.url} download={asset.name} className="m-1">
                                    ...
                                    <Badge variant="secondary" className="absolute bottom-1 left-1 text-xs px-1 py-0.5 opacity-80">
                                      {asset.name}
                                    </Badge>
                                  </div>
                                ))}
                              ...
                              <p className="text-sm text-gray-500 text-center">
                                {isSpanish ? "Aún no hay recursos en esta categoría." : "No assets in this category yet."}
                              </p>
                            )}
                          </AccordionContent>
                        </AccordionItem>
              ...
              <div className="flex gap-3 pt-6">
                <Button type="button" variant="outline" onClick={handlePrevStep} className="flex-1" data-testid="button-prev-step-3">
                  <ArrowLeft className="w-4 h-4 mr-2" />
                  {isSpanish ? "Anterior" : "Previous"}
                </Button>
                <Button type="button" onClick={handleNextStep} className="flex-1" data-testid="button-next-step-3">
                  {isSpanish ? "Siguiente" : "Next"}
                  <ArrowRight className="w-4 h-4 ml-2" />
                </Button>
              </div>
            ...
            <CardContent className="space-y-6">
              ...
              <div className="grid gap-6">
                {Object.entries(INTEGRATION_CATEGORIES).map(([key, category]) => {
                  ...
                  const providersInCategory = Object.entries(INTEGRATION_PROVIDERS).filter(
                    ([, provider]) => provider.category === key
                  );
                    ...
                    <Card key={key} className="border">
                      ...
                      <CardContent>
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                          {providersInCategory.map(([providerKey, provider]) => {
                            ...
                            return (
                              ...
                              >
                                <ProviderIcon className="w-8 h-8 text-gray-600" />
                                <div>
              ...
              <div className="flex gap-3 pt-6">
                <Button type="button" variant="outline" onClick={handlePrevStep} className="flex-1" data-testid="button-prev-step-4">
                  <ArrowLeft className="w-4 h-4 mr-2" />
                  {isSpanish ? "Anterior" : "Previous"}
                </Button>
                <Button type="button" onClick={handleFinishOnboarding} className="flex-1 bg-green-600 hover:bg-green-700" data-testid="button-finish-onboarding">
