...
export default function CustomersPage() {
  ...
  const createCustomerMutation = useMutation({
    mutationFn: async (data: any) => apiRequest('/api/customers', { method: 'POST', body: data }),
    onSuccess: () => {
  ...
  const createInvoiceMutation = useMutation({
    mutationFn: async (data: any) => apiRequest('/api/invoices', { method: 'POST', body: data }),
    onSuccess: () => {
  ...
  const updateInvoiceMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: any }) => 
      apiRequest(`/api/invoices/${id}`, { method: 'PUT', body: data }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/invoices'] });
  ...
  const handleFileUploadComplete = (invoiceId: string) => (result: UploadResult<Record<string, unknown>, Record<string, unknown>>) => {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold text-gray-900" data-testid="text-page-title">Customer Management</h1>
          <p className="text-gray-600">Manage your customers and their invoices</p>
        </div>
          ...
          <DialogContent>
            ...
            <form onSubmit={handleAddCustomer} className="space-y-4">
              <div>
                ...
                <Input id="name" name="name" required data-testid="input-customer-name" />
              </div>
              <div>
                <Label htmlFor="email">Email</Label>
                <Input id="email" name="email" type="email" data-testid="input-customer-email" />
              </div>
              <div>
                <Label htmlFor="phone">Phone</Label>
                <Input id="phone" name="phone" data-testid="input-customer-phone" />
              </div>
              <div>
                <Label htmlFor="company">Company</Label>
                <Input id="company" name="company" data-testid="input-customer-company" />
              </div>
              <div>
                <Label htmlFor="address">Address</Label>
                <Textarea id="address" name="address" data-testid="input-customer-address" />
              </div>
              <div>
                <Label htmlFor="notes">Notes</Label>
                <Textarea id="notes" name="notes" data-testid="input-customer-notes" />
              </div>
              <div>
              ...
              <div className="flex justify-end space-x-2">
                <Button type="button" variant="outline" onClick={() => setShowAddCustomer(false)} data-testid="button-cancel-customer">
                  Cancel
                </Button>
            ...
            <TableBody>
              {customers.map((customer: Customer) => (
                <TableRow key={customer.id} data-testid={`row-customer-${customer.id}`}>
                  ...
                  <TableCell data-testid={`text-customer-company-${customer.id}`}>
                    {customer.company || "-"}
                  </TableCell>
                  ...
                  <TableCell data-testid={`text-customer-total-${customer.id}`}>
        ...
        <Card>
          ...
          <CardContent>
            <Table>
              ...
              <TableBody>
                {invoices
                  .filter((invoice: InvoiceWithCustomer) => invoice.customerId === selectedCustomer.id)
                    ...
                    <TableRow key={invoice.id} data-testid={`row-invoice-${invoice.id}`}>
                      ...
                      <TableCell>
                        <ObjectUploader
                          ...
                          onGetUploadParameters={() => handleFileUpload(invoice.id)}
                          onComplete={handleFileUploadComplete(invoice.id)}
      ...
      <Dialog open={showAddInvoice} onOpenChange={setShowAddInvoice}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Add Invoice for {selectedCustomer?.name}</DialogTitle>
            <DialogDescription>
          ...
          <form onSubmit={handleAddInvoice} className="space-y-4">
            <div>
              ...
              <Input id="invoiceNumber" name="invoiceNumber" required data-testid="input-invoice-number" />
            </div>
            <div>
              <Label htmlFor="amount">Amount (USD)</Label>
              <Input id="amount" name="amount" type="number" step="0.01" required data-testid="input-invoice-amount" />
            </div>
            <div>
              <Label htmlFor="description">Description</Label>
              <Textarea id="description" name="description" data-testid="input-invoice-description" />
            </div>
            <div>
              <Label htmlFor="dueDate">Due Date</Label>
              <Input id="dueDate" name="dueDate" type="date" data-testid="input-invoice-due-date" />
            </div>
            <div>
              <Label htmlFor="status">Status</Label>
            ...
            <div className="flex justify-end space-x-2">
              <Button type="button" variant="outline" onClick={() => setShowAddInvoice(false)} data-testid="button-cancel-invoice">
                Cancel
              </Button>
